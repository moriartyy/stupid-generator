...
import stupid.generator.util.sqlparse.NamedParameterUtils;
import stupid.generator.util.sqlparse.ParsedSql;
...
public class SqlSegment {
	...
	public Set<SqlParameter> getParams(Sql sql) {
		...
		for (String paramName : getParamNames()) {
			...
			if (p == null)
				throw new IllegalArgumentException("not found param on sql:" + parsedIncludeSql + " with name:" + paramName + " for sqlSegment:" + id); //是否不该扔异常
			result.add(p);
		}
		return result;
	}
