 ...
 */
package stupid.generator.provider.java.model;

import stupid.generator.util.GLogger;
import stupid.generator.util.StringHelper;
import stupid.generator.util.typemapping.JavaImport;
...
public class JavaMethod {
	...
	public static class JavaMethodInvokeSequencesParser {
		...
		private String getMethodBody(String javaSourceContent) {
			...
			String methodStartPattern = "(?s)\\s+" + method.getMethodName() + "\\s*\\(" + MethodParameter.JavaSourceFileMethodParametersParser.getSimpleParamsPattern(method.method) + "\\)\\s*";

			int methodStart = StringHelper.indexOfByRegex(javaSourceContent, methodStartPattern);
			if (methodStart == -1)
				throw new IllegalArgumentException("cannot get method body by pattern:" + methodStartPattern + " methodName:" + method.getMethodName() + "\n javaSource:" + javaSourceContent);
